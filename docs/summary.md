...
QR コード誘導 → 回答ボリュームに応じた 3 ルートのアンケート → Gemini API を使った口コミ生成 → Google マップ投稿導線までをスマートフォン完結で支援します。

---

## 1. プロジェクト概要
- 店頭やイベントで配布する QR コードから、**ライト／ミディアム／フルシティ**（初級／中級／上級）の 3 ルートへ誘導し、回答負荷に応じた UX を提供する。
...
## 2. ユースケースと主要画面フロー
### QR 誘導〜アンケート
- `/` ルータ: 3 種のカップカードを表示。タップすると `/.netlify/functions/distribute` がローテーションしたフォーム URL を返却し、ユーザーを `form1`〜`form3` へ振り分ける。
- `form1`〜`form3`: 管理画面の設問設定（タイプ、必須、複数回答可、星評価 ON/OFF、プレースホルダー、口コミ反映フラグ）を読み込み動的に表示。送信時は設定済み API へ POST し、成功後に対応する生成ページへ遷移。

### 口コミ生成ページ `/generator`, `/generator/page2`, `/generator/page3`
- 各ページが `data-prompt-key`・`data-tier` を元に `/.netlify/functions/generate` を呼び出し、GAS 経由で取得した回答データ＋保存済みプロンプトを Gemini API（v1）へ連携。
...
- **プロンプトジェネレーター**: 口コミ生成用のプロンプト、ライト／スタンダード／プラチナ例文、サンプル入力をまとめて保管。Gemini API と直結しブラウザ上で試行できる。

### ユーザー設定 `/user`
- **ユーザー情報タブ**で店舗名、カタカナ表記、業種、客層、強み、検索で押さえたいキーワード／除外ワード、駅近フラグ、Google マップ口コミリンクを保存。
- 運用担当が頻繁に編集する要素（トップページ文言、アンケート設定、AI プロンプト、QR 生成）を 1 画面に集約し、システム寄りの情報は `/admin` に隔離。
...
- `distribute`: 3 カードの遷移先 URL をラウンドロビンで返却し、均等誘導を実現。
...
- `user-data-submit`: `/user` で編集したプロフィールやキーワード群を Blobs へ保存。
- `prompt-generator`: ブラウザ上から Gemini API を叩いてプロンプトの試行錯誤を行う補助 API。
...
## 4. 設定項目とデータ構造
### ルータ & ランディング
- カードごとにタイトル、説明、ボタンテキスト、カラー、遷移先、配信順、再配信ルールを保持。イベント単位の AB テストが容易。
- `/admin` で保存後すぐにルータへ反映されるため、デプロイなしで現場調整が可能。

### アンケートフォーム定義
- `form1`（ライト）: 1〜2 問の星評価＋自由記述を想定。`rating` / `text` タイプ中心。
- `form2`（ミディアム）: ドロップダウン、チェックボックスなど複数回答可の設問を設定。`allowMultiple`、`includeInReview` を個別制御。
- `form3`（フルシティ）: 詳細ヒアリング用に rating + text の組み合わせを採用。
- 各質問は `id` / `title` / `required` / `type` / `options` / `ratingStyle` / `includeInReview` を持ち、保存時にサニタイズされる。

### プロンプト＆ Gemini 設定
- `prompt-generator` タブで、ライト／スタンダード／プラチナ向けの例文と参照フレーズを登録。運用チームが入力値を試しながら最適化できる。
